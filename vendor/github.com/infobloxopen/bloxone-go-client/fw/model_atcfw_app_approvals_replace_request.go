/*
BloxOne FW API

BloxOne Threat Defense Cloud is an extension of the BloxOne Suite that provides visibility into infected and compromised off-premises devices, roaming users, remote sites, and branch offices. You can subscribe to BloxOne Cloud and use its functionality to mitigate and control malware as well as provide unprecedented insight into your network security posture and enable timely action. BloxOne Cloud also offers unified policy management, reporting, and threat analytics across the entire spectrum. Using automated and high-quality threat intelligence feeds and unique behavioral analytics, it automatically stops device communications with C&Cs/botnets and prevents DNS based data exfiltration.  The mission-critical DNS infrastructure can become a vulnerable component in your network when it is inadequately protected by traditional security solutions and consequently used as an attack surface. Compromised DNS services can result in catastrophic network and system failures. To fully protect your network in today’s cyber security threat environment, Infoblox sets a new DNS security standard by offering scalable, enterprise-grade, and integrated protection for your DNS infrastructure.  Through the Infoblox Cloud Services Portal, you can view the status of your subscription and threat intelligence feeds, manage your network scope and roaming end users, and learn more about threats on your networks through the Infoblox Threat Lookup tool and predefined reports.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fw

import (
	"encoding/json"
)

// checks if the AtcfwAppApprovalsReplaceRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AtcfwAppApprovalsReplaceRequest{}

// AtcfwAppApprovalsReplaceRequest struct for AtcfwAppApprovalsReplaceRequest
type AtcfwAppApprovalsReplaceRequest struct {
	Approvals []AtcfwAppApproval `json:"approvals,omitempty"`
	Fields    *ProtobufFieldMask `json:"fields,omitempty"`
}

// NewAtcfwAppApprovalsReplaceRequest instantiates a new AtcfwAppApprovalsReplaceRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAtcfwAppApprovalsReplaceRequest() *AtcfwAppApprovalsReplaceRequest {
	this := AtcfwAppApprovalsReplaceRequest{}
	return &this
}

// NewAtcfwAppApprovalsReplaceRequestWithDefaults instantiates a new AtcfwAppApprovalsReplaceRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAtcfwAppApprovalsReplaceRequestWithDefaults() *AtcfwAppApprovalsReplaceRequest {
	this := AtcfwAppApprovalsReplaceRequest{}
	return &this
}

// GetApprovals returns the Approvals field value if set, zero value otherwise.
func (o *AtcfwAppApprovalsReplaceRequest) GetApprovals() []AtcfwAppApproval {
	if o == nil || IsNil(o.Approvals) {
		var ret []AtcfwAppApproval
		return ret
	}
	return o.Approvals
}

// GetApprovalsOk returns a tuple with the Approvals field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AtcfwAppApprovalsReplaceRequest) GetApprovalsOk() ([]AtcfwAppApproval, bool) {
	if o == nil || IsNil(o.Approvals) {
		return nil, false
	}
	return o.Approvals, true
}

// HasApprovals returns a boolean if a field has been set.
func (o *AtcfwAppApprovalsReplaceRequest) HasApprovals() bool {
	if o != nil && !IsNil(o.Approvals) {
		return true
	}

	return false
}

// SetApprovals gets a reference to the given []AtcfwAppApproval and assigns it to the Approvals field.
func (o *AtcfwAppApprovalsReplaceRequest) SetApprovals(v []AtcfwAppApproval) {
	o.Approvals = v
}

// GetFields returns the Fields field value if set, zero value otherwise.
func (o *AtcfwAppApprovalsReplaceRequest) GetFields() ProtobufFieldMask {
	if o == nil || IsNil(o.Fields) {
		var ret ProtobufFieldMask
		return ret
	}
	return *o.Fields
}

// GetFieldsOk returns a tuple with the Fields field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AtcfwAppApprovalsReplaceRequest) GetFieldsOk() (*ProtobufFieldMask, bool) {
	if o == nil || IsNil(o.Fields) {
		return nil, false
	}
	return o.Fields, true
}

// HasFields returns a boolean if a field has been set.
func (o *AtcfwAppApprovalsReplaceRequest) HasFields() bool {
	if o != nil && !IsNil(o.Fields) {
		return true
	}

	return false
}

// SetFields gets a reference to the given ProtobufFieldMask and assigns it to the Fields field.
func (o *AtcfwAppApprovalsReplaceRequest) SetFields(v ProtobufFieldMask) {
	o.Fields = &v
}

func (o AtcfwAppApprovalsReplaceRequest) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AtcfwAppApprovalsReplaceRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Approvals) {
		toSerialize["approvals"] = o.Approvals
	}
	if !IsNil(o.Fields) {
		toSerialize["fields"] = o.Fields
	}
	return toSerialize, nil
}

type NullableAtcfwAppApprovalsReplaceRequest struct {
	value *AtcfwAppApprovalsReplaceRequest
	isSet bool
}

func (v NullableAtcfwAppApprovalsReplaceRequest) Get() *AtcfwAppApprovalsReplaceRequest {
	return v.value
}

func (v *NullableAtcfwAppApprovalsReplaceRequest) Set(val *AtcfwAppApprovalsReplaceRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableAtcfwAppApprovalsReplaceRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableAtcfwAppApprovalsReplaceRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAtcfwAppApprovalsReplaceRequest(val *AtcfwAppApprovalsReplaceRequest) *NullableAtcfwAppApprovalsReplaceRequest {
	return &NullableAtcfwAppApprovalsReplaceRequest{value: val, isSet: true}
}

func (v NullableAtcfwAppApprovalsReplaceRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAtcfwAppApprovalsReplaceRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
