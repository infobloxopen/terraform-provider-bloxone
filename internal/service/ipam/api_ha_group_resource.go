...
func (r *HaGroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	apiRes, _, err := r.client.IPAddressManagementAPI.
		...
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create HaGroup, got error: %s", err))
		return
	}
...
func (r *HaGroupResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	apiRes, _, err := r.client.IPAddressManagementAPI.
		...
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update HaGroup, got error: %s", err))
		return
	}
...
func (r *HaGroupResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete HaGroup, got error: %s", err))
		return
	}
}
