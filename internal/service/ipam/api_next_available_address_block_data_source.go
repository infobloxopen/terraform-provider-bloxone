	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"tag_filters": schema.MapAttribute{
				ElementType:         types.StringType,
				Optional:            true,
				MarkdownDescription: "Filter address blocks by tags. Key-value pairs to match.",
			},
		},
	}
}
...
func (d *NextAvailableAddressBlockDataSource) buildTagFilterString(tagFilters map[string]types.String) string {
	if len(tagFilters) == 0 {
		return ""
	}
...
func (d *NextAvailableAddressBlockDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if len(tagFilters) > 0 {
		...
		for {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list AddressBlocks by tags, got error: %s", err))
				return
			}
		...
		if int32(len(findResults)) < count {
			resp.Diagnostics.AddError(
				...
				fmt.Sprintf("Requested %d Address Blocks with CIDR %d, but only %d were found. Not enough Address Blocks available across all checked address blocks.", count, data.Cidr.ValueInt64(), len(findResults)),
			)
			return
		}
