...
func TestAccSubnetDataSource_Filters(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSubnetDataSourceConfigFilters(spaceName, "11.0.0.0", 24),
...
func TestAccSubnetDataSource_TagFilters(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSubnetDataSourceConfigTagFilters(spaceName, "11.0.0.0", 24, acctest.RandomName()),
				Check: resource.ComposeTestCheckFunc(
					append([]resource.TestCheckFunc{
						testAccCheckSubnetExists(context.Background(), resourceName, &v),
					}, testAccCheckSubnetResourceAttrPair(resourceName, dataSourceName)...)...,
...
func testAccCheckSubnetResourceAttrPair(resourceName, dataSourceName string) []resource.TestCheckFunc {
	return []resource.TestCheckFunc{
		...
		resource.TestCheckResourceAttrPair(resourceName, "config_profiles", dataSourceName, "results.0.config_profiles"),
		...
		resource.TestCheckResourceAttrPair(resourceName, "external_keys", dataSourceName, "results.0.external_keys"),
		resource.TestCheckResourceAttrPair(resourceName, "federation_realms", dataSourceName, "results.0.federation_realms"),
...
func testAccSubnetDataSourceConfigFilters(spaceName, address string, cidr int) string {
	config := fmt.Sprintf(`
...
func testAccSubnetDataSourceConfigTagFilters(spaceName, address string, cidr int, tagValue string) string {
	config := fmt.Sprintf(`
resource "bloxone_ipam_subnet" "test" {
    address = %q
    cidr = %d
