...
func (m *IpamsvcDHCPInfoModel) Flatten(ctx context.Context, from *ipam.DHCPInfo, diags *diag.Diagnostics) {
	...
	m.ClientHostname = flex.FlattenStringPointer(from.ClientHostname)
	m.ClientHwaddr = flex.FlattenStringPointer(from.ClientHwaddr)
	m.ClientId = flex.FlattenStringPointer(from.ClientId)
	m.End = timetypes.NewRFC3339TimePointerValue(from.End)
	m.Fingerprint = flex.FlattenStringPointer(from.Fingerprint)
	m.Iaid = flex.FlattenInt64Pointer(from.Iaid)
	m.LeaseType = flex.FlattenStringPointer(from.LeaseType)
	m.PreferredLifetime = timetypes.NewRFC3339TimePointerValue(from.PreferredLifetime)
	m.Remain = flex.FlattenInt64Pointer(from.Remain)
