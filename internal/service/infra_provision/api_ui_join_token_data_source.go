...
var _ datasource.DataSource = &UIJoinTokenDataSource{}

func NewUIJoinTokenDataSource() datasource.DataSource {
	return &UIJoinTokenDataSource{}
}
...
func (d *UIJoinTokenDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "infra_join_tokens"
}
...
func (m *HostactivationJoinTokenModelWithFilter) FlattenResults(ctx context.Context, from []infra_provision.HostactivationJoinToken, diags *diag.Diagnostics) {
	...
	m.Results = flex.FlattenFrameworkListNestedBlock(ctx, from, HostactivationJoinTokenAttrTypes, diags, FlattenHostactivationJoinToken)
}

func (d *UIJoinTokenDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Host Activation Join Tokens.\n\nA join token is random character string which is used for instant validation of new hosts.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(HostactivationJoinTokenResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
		},
