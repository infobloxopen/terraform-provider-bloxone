...
var _ datasource.DataSource = &NamedListsDataSource{}

func NewNamedListsDataSource() datasource.DataSource {
	return &NamedListsDataSource{}
}
...
func (d *NamedListsDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "td_named_lists"
}
...
func (d *NamedListsDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NamedListsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data AtcfwNamedListModelWithFilter
	...
	allResults, err := utils.ReadWithPages(func(offset, limit int32) ([]fw.NamedListRead, error) {
		apiRes, _, err := d.client.FWAPI.
			...
			Execute()
		if err != nil {
			resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Named List, got error: %s", err))
			return nil, err
		}
