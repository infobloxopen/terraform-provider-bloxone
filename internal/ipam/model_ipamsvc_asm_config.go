...
var IpamsvcASMConfigResourceSchema = schema.Schema{
...
func (m *IpamsvcASMConfigModel) expand(ctx context.Context, diags *diag.Diagnostics) *ipam.IpamsvcASMConfig {
	...
	to := &ipam.IpamsvcASMConfig{
		AsmThreshold:       ptr(int64(m.AsmThreshold.ValueInt64())),
		Enable:             m.Enable.ValueBoolPointer(),
		EnableNotification: m.EnableNotification.ValueBoolPointer(),
		ForecastPeriod:     ptr(int64(m.ForecastPeriod.ValueInt64())),
		GrowthFactor:       ptr(int64(m.GrowthFactor.ValueInt64())),
		GrowthType:         m.GrowthType.ValueStringPointer(),
		History:            ptr(int64(m.History.ValueInt64())),
		MinTotal:           ptr(int64(m.MinTotal.ValueInt64())),
		MinUnused:          ptr(int64(m.MinUnused.ValueInt64())),
...
func (m *IpamsvcASMConfigModel) flatten(ctx context.Context, from *ipam.IpamsvcASMConfig, diags *diag.Diagnostics) {
	...
	m.AsmThreshold = types.Int64Value(int64(*from.AsmThreshold))
	m.Enable = types.BoolPointerValue(from.Enable)
	m.EnableNotification = types.BoolPointerValue(from.EnableNotification)
	m.ForecastPeriod = types.Int64Value(int64(*from.ForecastPeriod))
	m.GrowthFactor = types.Int64Value(int64(*from.GrowthFactor))
	m.GrowthType = types.StringPointerValue(from.GrowthType)
	m.History = types.Int64Value(int64(*from.History))
	m.MinTotal = types.Int64Value(int64(*from.MinTotal))
	m.MinUnused = types.Int64Value(int64(*from.MinUnused))
