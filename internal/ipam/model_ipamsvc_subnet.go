...
var IpamsvcSubnetResourceSchema = schema.Schema{
	MarkdownDescription: `A __Subnet__ object (_ipam/subnet_) is a set of contiguous IP addresses in the same IP space with no gap, expressed as an address and CIDR values. It represents a set of addresses from which addresses are assigned to network equipment interfaces.`,
...
func expandIpamsvcSubnet(ctx context.Context, o types.Object, diags *diag.Diagnostics) *ipam.IpamsvcSubnet {
	if o.IsNull() || o.IsUnknown() {
		return nil
	}

	var m IpamsvcSubnetModel
...
func (m *IpamsvcSubnetModel) expand(ctx context.Context, diags *diag.Diagnostics) *ipam.IpamsvcSubnet {
	if m == nil {
		return nil
	}
	...
	to := &ipam.IpamsvcSubnet{
		...
		RebindTime:                 ptr(int64(m.RebindTime.ValueInt64())),
		RenewTime:                  ptr(int64(m.RenewTime.ValueInt64())),
...
func (m *IpamsvcSubnetModel) flatten(ctx context.Context, from *ipam.IpamsvcSubnet, diags *diag.Diagnostics) {
	...
	m.RebindTime = types.Int64Value(int64(*from.RebindTime))
	m.RenewTime = types.Int64Value(int64(*from.RenewTime))
