...
var IpamsvcKerberosKeyResourceSchema = schema.Schema{
	MarkdownDescription: `A __KerberosKey__ object (_keys/kerberos_) represents a Kerberos key.`,
...
func (m *IpamsvcKerberosKeyModel) flatten(ctx context.Context, from *ipam.IpamsvcKerberosKey, diags *diag.Diagnostics) {
	...
	m.Algorithm = types.StringPointerValue(from.Algorithm)
	m.Domain = types.StringPointerValue(from.Domain)
	m.Key = types.StringValue(from.Key)
	m.Principal = types.StringPointerValue(from.Principal)
	m.UploadedAt = types.StringPointerValue(from.UploadedAt)
	m.Version = types.Int64Value(int64(*from.Version))
