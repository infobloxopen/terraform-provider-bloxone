...
page_title: "bloxone_dns_txt_record Resource - terraform-provider-bloxone"
subcategory: "DNS"
description: |-
  Manages a DNS TXT resource record in an authoritative zone.
---

# bloxone_dns_txt_record (Resource)
...
- `subtype` (String) The DNS resource record subtype specified in the textual mnemonic format. Valid only in case _type_ is _IBMETA_.
  ...
  | LBDN  | 1            | LBDN record   |
  <br>
- `type` (String) The type of DNS record. This is always `TXT`.
